...
fn depcache_load(mame: &str, dotdir: &PathBuf, _stdout: &Term) -> Result<Table, String> {
    ...
    let mamever = Exec::shell(format!(r#"{} -version"#, mame)).capture().expect("!!").stdout_str();
    println!("Running on MAME {}", style(mamever.clone()).bold().cyan());
...
fn depcache_init(mame: &str, dotdir: &PathBuf, stdout: &Term) -> Result<Table, bool> {
